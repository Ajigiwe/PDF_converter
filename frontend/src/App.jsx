...
export default function App() {
  ...
  const uploadFiles = async () => {
    ...
    if (!res.ok) throw new Error('Failed to upload files to server');
    ...
    <div style={{ padding: '1.5rem', fontFamily: 'var(--font-sans)', color: 'var(--color-text-primary)' }}>
      ...
      <div className="settings-panel">
        ...
        <div className="settings-grid">
          ...
          {mode === 'merge' && convertOptions.outputFormat === 'pdf' && (
            <div className="setting-row">
              <label>Pages per Sheet</label>
              <select 
                value={convertOptions.pagesPerSheet}
                onChange={(e) => setConvertOptions({...convertOptions, pagesPerSheet: e.target.value})}
              >
                <option value="1">1-up (Original)</option>
      ...
      <div className="action-buttons">
        {mode === 'convert' ? (
          ...
          >
            {loading ? '⏳ Converting...' : `🔄 Convert to ${convertOptions.outputFormat.toUpperCase()}`}
          </button>
        ) : (
          <button 
            className="action-btn primary"
